        ...
        <div className="grid md:grid-cols-3 gap-8">
          {plans.map((plan, idx) => (
            ...
            >
              ...
              <h4 className="text-xl font-bold italic mb-2">
                {plan.title}
              </h4>
              <p className={`text-[10px] font-bold uppercase mb-8 ${
              ...
              <ul className={`text-xs space-y-4 grow italic mb-10 ${
                idx === 1 ? 'text-off-white/60' : 'text-soft-brown dark:text-off-white/70'
              }`}>
                {(plan.features as string[]).map((feature, fidx) => (
                  <li key={fidx} className="flex items-center gap-2">
                    <Check className={`w-4 h-4 shrink-0 ${idx === 1 ? 'text-warm-gold' : 'text-warm-gold'}`} />
                    {feature}
                  </li>
                ))}
              </ul>
