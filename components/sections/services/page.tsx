                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {services.map(({ icon: Icon, key, title, description }, index, arr) => {
                        ...
                        return (
                            <div
                                key={key}
                                className={`p-10 rounded-[40px] bg-off-white dark:bg-soft-brown border border-dark-umber/5 dark:border-off-white/10 hover:border-warm-gold/30 dark:hover:border-warm-gold/30 transition-all duration-500 group hover:-translate-y-1 ${isOddCardAlone ? 'md:col-span-2 lg:col-span-1' : ''}`}
                            >
